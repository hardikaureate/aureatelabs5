    ...
    <div class="blog-detail-inner">
        <div class="guide-side-panel">
            <div class="guide-title">
                <?php
                if (get_post_parent(get_the_ID())) {
                    $parentData = get_post_parent(get_the_ID());
                    if ($parentData) {
                        $parentTitle = $parentData->post_title;
                        $parentID = $parentData->ID;
                        $parentLink = get_the_permalink($parentID);
                        echo '<a href="' . $parentLink . '">' . $parentTitle . '</a>';
                        $is_parent = false;
                    }
                } else {
                    $parentTitle = get_the_title(get_the_ID());
                    $parentID = get_the_ID();
                    $parentLink = get_the_permalink($parentID);
                    echo '<a href="' . $parentLink . '">' . $parentTitle . '</a>';
                    $is_parent = true;
            ...
            <?php
            $args = array(
                'post_type' => 'guides', 'posts_per_page' => -1,  'no_found_rows'  => true, 'post_parent' => $parentID, 'orderby' => 'menu_order', 'order' => 'ASC'
            );
            ...
            ?>
                <div class="guide-menus">
                    ...
                    foreach ($wpex_query->posts as $post) : setup_postdata($post);
                        ...
                        if ($is_parent && $count == 1) {
                            $nextPostID = get_the_ID();
                        }
                        if ($is_parent == false && $count == 1) {
                            $prevPostID = $parentID;
                        }
                        $count++;
                    ?>
            ...
            <?php else : ?>
                <div class="no-guide-menus">
                    <?php _e('Sorry, there is no more chapter.'); ?>
                </div>
            <?php endif; ?>
        ...
        <div class="guide-content-panel">
            <div class="guide-content-panel-inner">
                ...
                <?php
                        ...
                        $next_post = $prevPostID;
                        $previous_post = $nextPostID;

                ?>
                <div class="guides_next_prev_group">
